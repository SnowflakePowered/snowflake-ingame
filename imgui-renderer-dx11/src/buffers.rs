...
use windows::Win32::Graphics::Direct3D11::{
    ID3D11Buffer, ID3D11Device, D3D11_BIND_FLAG, D3D11_BIND_INDEX_BUFFER, D3D11_BIND_VERTEX_BUFFER,
    ...
    ImGuiBuffer<T, { ADD_CAPACITY }, { BIND_FLAGS }>
{
    ...
    pub fn reserve(&mut self, count: usize) -> HResult<()> {
        if self.len() < count {
            let (buffer, len) = Self::create_buffer(&self.device, count)?;
            self.buffer = buffer;
            self.len = len;
        }
        Ok(())
...
pub type VertexBuffer =
    ImGuiBuffer<imgui::DrawVert, VERTEX_BUF_ADD_CAPACITY, { D3D11_BIND_VERTEX_BUFFER.0 }>;
pub type IndexBuffer =
    ImGuiBuffer<imgui::DrawIdx, INDEX_BUF_ADD_CAPACITY, { D3D11_BIND_INDEX_BUFFER.0 }>;
