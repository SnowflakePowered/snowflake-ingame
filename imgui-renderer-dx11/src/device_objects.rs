...
impl FontTexture {
    pub fn tex_id(&self) -> imgui::TextureId {
        unsafe { TextureId::from(std::mem::transmute_copy::<_, *const ()>(&self.font_resource_view) as usize) }
    }

    pub fn new(fonts: &mut imgui::FontAtlasRefMut<'_>, device: &ID3D11Device) -> HResult<FontTexture> {
